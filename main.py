...
app = Flask(__name__)
BOT_TOKEN = "ТВОЙ_ТОКЕН_БОТА"  # <-- ВСТАВЬ СВОЙ ТОКЕН
...
def init_db():
    ...
    cursor = conn.cursor()
    # Храним ID, кол-во прокрутов за сегодня и дату последнего прокрута
...
def get_user_stats(user_id):
    ...
    if not res or res[1] != today:
        return 0, today  # Если новый день или новый юзер — 0 прокрутов
    return res[0], today
...
def create_spin_invoice():
    ...
    if count >= 3:
        return jsonify({"error": "Лимит 3 прокрута в день исчерпан!"}), 400

    # Создаем инвойс на 6 звезд
    payload = {
        "title": "Один прокрут NFT колеса",
        "description": f"Попытка №{count + 1} за сегодня",
        "payload": f"spin_{user_id}_{count}_{today}",
        "currency": "XTR",
        "prices": [{"label": "Spin", "amount": 6}]
    }

    url = f"https://telegram.org{8723694663:AAEKRDMJ3JvrNDMUR_6vc12ztF8npLFdO54}/createInvoiceLink"
...
# Этот маршрут вызывается из JS только ПОСЛЕ успешной оплаты
